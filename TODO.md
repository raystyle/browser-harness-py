...
| v0.6.11/v0.6.12 双发 | 已完成 | 用户令「封板发布→本地更新→最后本地验收」：v0.6.11（wait_for_render 渲染判官 + X 供给窗分片 + S009 研究）预检 296 绿→tag/Release→装机更新；验收矩阵当场抓出 rAF 缺陷（静态页 frames 恒 0 误判冻死），心跳改 setInterval 修复即追发 v0.6.12；重跑验收全绿（隔离 Google 3 条 / 静态页 render 0.7s / doctor 4ok0FAIL / 版本对齐）。极限测试同场：Google 翻页 5 页 48 uniq 零墙、X 单查询供给窗 ~10-20 条（热词亦然）+ since/until 分片破窗（两窗 27 uniq） | 2026-09-04 |
