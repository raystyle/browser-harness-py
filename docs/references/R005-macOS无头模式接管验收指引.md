...
v0.6.6 的无头能力（`BH_CHROME_HEADLESS`、`BH_AGENT_CDP_PORT`）与 cookies 插件**只存在于 Windows 开发机的本地提交里，origin 还是 v0.6.5**（发版被暂缓）。Mac 拿到代码三条路任选：
...
- 无头 UA `HeadlessChrome` 对 X 时间线无影响 [实证: WSL 11 分钟 +27 推、Windows 两轮 +2]；个别反爬站点用 `BH_CHROME_EXTRA_FLAGS=--user-agent=...` 覆盖。
